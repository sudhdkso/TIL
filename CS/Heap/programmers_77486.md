...
```
### **문제 풀이** 
- 트리의 parent를 각 Node에서 확인할 수 있도록 Node 클래스를 만들었다.
- 그리고 child Node를 생성할 때 parent Node를 찾기 위해서 각 Node를 list에 넣음과 동시에 Map에 name을 key로 index를 value로 해서 넣어주었다.
    - 이렇게 map을 별도로 사용한 이유는 list의 indexOf()함수의 속도가 생각보다 오래 걸리기 때문에 시간초과가 발생할 수 있다. 따라서 name을 통해서 index를 바로 찾기위해서 map을 사용하였다.

- 위의 과정을 통해 각 판매자의 추천인들까지 모두 list에 넣으면, 입력받은 수익에 대해서 정산을 해준다.
- 이때 최초판매자의 수익의 10%는 상위 추천인들에게 전달되어야한다.
- 이 부분을 각 수익과 판매자명을 매개변수로 받는 `setProfit()`함수를 통해서 구현해주었다.
    - `setProfit()`은 최초판매자의 이름과 칫솔의 판매갯수를 매개변수로 받는다.
    - 그리고 입력받은 판매자의 이름을 map에서 찾아 index를 구한다. 그리고 그 index를 통해서 전체 수익의 90%를 result[index]에 더하고, 상위 추천인을 찾는다.
        - 이때 수익을 10%가 한자리수 미만이면 90%가 아닌 전액을 result[index]에 더하고 반복문을 탈출한다.
    - 그리고 money를 최초수익의/10으로 설정한다.
    - 위의 과정을 추천인의 상위 추천인이 없거나, 수익/10이 0보다 작아지면 멈춘다.
- 모든 amount에 대해서 위의 `setProfit()`함수를 통해서 이익을 분배하면 result에 각 i번째 사람이 얼마를 버는지 저장이 된다.
- 그리고 그 result배열을 solution에서 return해준다.
