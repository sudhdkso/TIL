...
class Solution {
    public int[] solution(int[] lottos, int[] win_nums) {
        ...
        for(int i=0;i<temp.size();i++){
            if(set.contains(temp.elementAt(i))){
                check++;
            }
        }
        answer[0] = rank[check+(6- temp.size())];
        answer[1] = rank[check];
...
```
- set은 당첨 번호인 win_num을 Set으로 저장한다.
- temp는 민우의 로또 번호 중에 0이 아닌애들만 저장을 하는 임의의 Vector이다.
- set에 temp가 있는 지 확인하고
### 출처
